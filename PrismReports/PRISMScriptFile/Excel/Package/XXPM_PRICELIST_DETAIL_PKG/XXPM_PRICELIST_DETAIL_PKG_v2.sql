...
BEGIN
  ...
  LOOP
    ...
    WHEN OTHERS THEN
      ...
      v_errm       := SQLCODE || '-' || SQLERRM;
    END;
    dbms_output.put_line('v_property_id-0-'||v_property_id);
    --------------------------------------------------------------------------------
...
--    END IF;
    ...
    END IF;
    if  i.PL_AMOUNT is null then  --i.min_price
         dbms_output.put_line('   min_price is null ');
        v_ready_flag := 'E';
        v_err_desc := 'Total Sale should not be  null ' ;
      xxdm_pl_lines_update(i.BATCH_ID,i.rowid,v_err_desc,v_ready_flag,v_errm);
...
--    UNIT_NUMBER
    ...
    FROM xxpm_property_area pa, xxpm_property_units pu
    where 
    pa.UNIT_ID=pu.UNIT_ID
    and rownum=1
    and pa.AREA='CHARGE' 
    and pu.UNIT_NUMBER=i.UNIT_NUMBER;
    exception when others then 
    L_AREASF:=0;
    end;
DBMS_OUTPUT.PUT_LINE('i.PL_AMOUNT   '||i.PL_AMOUNT);
DBMS_OUTPUT.PUT_LINE('L_AREASF   '||L_AREASF);

    UPDATE xxpm_pl_lines
    SET UOM            =i.UOM,
      PL_AMOUNT        =i.PL_AMOUNT,
      BASE_PRICE        =ROUND((nvl(i.PL_AMOUNT, 0)/nvl(L_AREASF, 0)), 2),
      MIN_PRICE         =ROUND((nvl(i.PL_AMOUNT, 0)/nvl(L_AREASF, 0)), 2)-500,
    ...
    AND rowid              = i.rowid; 
      ...
      INTO xxpm_pl_lines
        ...
        (
          ...
          v_unit_id,
          i.UOM,
          ROUND((nvl(i.PL_AMOUNT, 0)/nvl(L_AREASF, 0)), 2),
          ROUND((nvl(i.PL_AMOUNT, 0)/nvl(L_AREASF, 0)), 2)-500,
          i.START_DATE,
          i.END_DATE,
          i.REVISION_NO,
          i.REVISION_DATE,
