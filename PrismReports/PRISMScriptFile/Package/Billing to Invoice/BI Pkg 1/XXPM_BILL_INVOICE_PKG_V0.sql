...
--------------------------------------------------------
--  DDL for Package Body XXPM_BILL_INVOICE_PKG
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "XXPM_BILL_INVOICE_PKG" 
IS
PRAGMA SERIALLY_REUSABLE;
    ...
    IF TO_CHAR(c_bh.usage) = 'S' THEN
      BEGIN
        ...
        LOOP
          ...
          DBMS_OUTPUT.PUT_LINE('l_inv_func_id ID   ' || l_inv_func_id);
          ...
          DBMS_OUTPUT.PUT_LINE('Next number:- ' || l_inv_number || 'Function id:- ' ||l_inv_func_id);
...
--              'AR'
          ...
          LOOP
            ...
            l_instalment_type  := cr1.installment_type;
              ...
              if l_instalment_type is not null then
      ...
      IF cr1.MILESTONE_TYPE = 'MS'
            THEN                    
l_tot_amt := (cr2.BOOKING_AMOUNT * nvl(cr1.installment_pct,0) )/100;
l_line_amount := l_tot_amt - (l_tot_amt * nvl(cr2.DISC_PCT ,0)  )/100;
l_tax_amount :=l_tot_amt*nvl(cr2.tax_perc,0) /100;
else
l_tax_amount:=cr1.tax_amount;

l_line_amount :=cr1.amount -l_tax_amount;
end if;
...
--              END IF;
    ...
    ELSE
      ...
      DBMS_OUTPUT.PUT_LINE('l_invoice_number ...'||l_invoice_number );
          ...
          IF (cr1.charge_type IS NOT NULL) THEN
            INSERT
            INTO xxpm_invoice_header
              (
                invoice_id,
...
--              l_installment_amount
              ...
              INTO xxpm_invoice_lines
                ...
                (
                  ...
                  NULL,
                  NULL,
                                    l_installment_amount,  --                  l_installment_amount,
                  NULL,                                ---cr2.tax_code,
NULL,--                  cr2.tax_perc,
null,--                  cr2.tax_amount,
                  cr1.charge_type,
...
PROCEDURE XXPM_CREATE_INVOICE_TXN(
...
select TERM_ID into l_term_id from xxstg_pay_terms where term_name='IMMEDIATE'; 
  ...
  LOOP
    ...
    DBMS_OUTPUT.PUT_LINE('Next number:- ' || L_INVOICE_NUMBER || 'Function id:- ' ||L_INV_FUNC_ID);
    BEGIN
      SELECT sysdate + NVL(TERM_DAYS,0)
      INTO L_DUE_DATE
    ...
    INTO xxpm_invoice_header
      ...
      (
        ...
        NULL,
        NULL,
        L_INVOICING_RULE,  --invoicing_rule
        L_ACNT_RULE,       --acnt_rule
        L_RULE_START_DATE, --RULE_START_DATE
        l_rule_end_date,   --rule_end_date
        NULL,
        NULL,
        NULL,
        NULL,
        ...
        NULL, --i.created_by,
        L_DATE,
        NULL, --i.last_updated_by,
        L_DATE,
        NULL --i.last_update_login
      );
    l_line_number := 0;
    FOR j IN cur_invoice_line
    (
      i.booking_id
    )
    LOOP
      l_invoice_line_id := xxpm_invoice_lines_s.NEXTVAL;
      L_LINE_NUMBER     := L_LINE_NUMBER + 1;
      IF i.charge_type  IS NOT NULL THEN
        BEGIN
          SELECT 'For '
      ...
      INTO xxpm_invoice_lines
        ...
        (
          ...
          J.UNIT_NUMBER,
          NULL,               -- p_billing_id,
          i.installment_type, --                cr1.installment_type,
          NULL,
          NULL,
          NULL,
          NULL,
