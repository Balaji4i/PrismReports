...
--==============================================================================
 ...
 l_pay_mode   :=XXPM_DM_LIB_PKG.get_pay_mode(I.PAYMENT_MODE,I.ORG_ID);
    ...
    update XXDM_SALES_RECEIPTS_TSQ
    set org_id =l_org_id ,
...
cursor receipt_insert_lines (P_BK_NUM IN VARCHAR2,P_REC_NUM IN VARCHAR2)
IS
select
--rowid,
  RECEIPT_NUMBER,
...
PAY_MODE_ID,
BANK_ACCOUNT_NUM
from XXDM_SALES_RECEIPTS_TSQ where status_flag = 'V' and BOOKING_NUMBER=P_BK_NUM and RECEIPT_NUMBER=P_REC_NUM ;
...
i.RECEIPT_AMOUNT,--        recommended_amount,
i.RECEIPT_AMOUNT,--        received_amount,
null,--        diff_in_days,
null,--        pay_rec_date,
null,--        pay_rev_date,
null,--        rct_type,
...
i.BANK_ACCOUNT_NUM--        bank_account_num
    ...
    WHEN OTHERS THEN
        ...
        dbms_output.put_line('CURSOR1'||P_ERR_CODE);
end;
For s in receipt_insert_lines (i.BOOKING_NUMBER,i.RECEIPT_NUMBER) LOOP


dbms_output.put_line('BOOKING_NUMBER'||i.BOOKING_NUMBER);
dbms_output.put_line('RECEIPT_NUMBER'||i.RECEIPT_NUMBER);
...
s.AMOUNT_APPLIED,--    amount_applied,
s.applied_date,--    applied_date,
...
UPDATE XXDM_SALES_RECEIPTS_TSQ
    SET status_flag      ='P'
    WHERE booking_number = i.booking_number        and receipt_number=i.receipt_number;

 EXCEPTION
    WHEN OTHERS THEN
        P_ERR_CODE := SQLCODE;
        P_ERR_MSG  := SUBSTR (SQLERRM, 1, 2000);
        UPDATE XXDM_SALES_RECEIPTS_TSQ set STATUS_FLAG='F',ERR_DESC=P_ERR_CODE || '-' || P_ERR_MSG
