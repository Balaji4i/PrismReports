--------------------------------------------------------
--  DDL for View XXPM_RECEIPT_V
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE VIEW "XXPM_RECEIPT_V" ("ROW_ID", "RECEIPT_ID", "RECEIPT_NUMBER", "RECEIPT_DATE", "ORG_ID", "BUSINESS_UNIT", "RECEIPT_TYPE", "PROPERTY_ID", "PROPERY_NAME", "PROPERY_NUMBER", "UNIT_ID", "UNIT_NAME", "UNIT_NUMBER", "BUILD_ID", "BUILING_NAME", "BUILING_NUMBER", "LEAD_ID", "CUST_ID", "CUSTOMER_NAME", "CUST_SITE_ID", "SITE_NUMBER", "SALESMAN_ID", "CURRENCY_CODE", "RECEIPT_AMOUNT", "RECEIPT_METHOD_NAME", "PAY_MODE", "PAY_REF_NUMBER", "PAY_REF_DATE", "BANK_NAME", "BANK_BRANCH_NAME", "STATUS", "DESCRIPTION", "RECOMMENDED_AMOUNT", "RECEIVED_AMOUNT", "DIFF_IN_DAYS", "PAY_REC_DATE", "PAY_REV_DATE", "RCT_TYPE", "RCT_DESC", "SOURCE_FUNC_ID", "SOURCE_FUNC_REF_ID") AS 
  SELECT  xr.ROWID
        ,xr.receipt_id
        ,xr.receipt_number
        ,xr.receipt_date
   ...
   WHERE     xr.org_id = xo.org_id
         ...
         AND NVL(xr.property_id,-99) =   NVL(xr.property_id,-99) +0
         AND NVL(xpu.unit_id ,-99) =   NVL(xpu.unit_id ,-99) +0
         AND NVL(xpb.build_id,-99) =   NVL(xpb.build_id,-99) +0
         AND xr.cust_id =   xr.cust_id+ 0
