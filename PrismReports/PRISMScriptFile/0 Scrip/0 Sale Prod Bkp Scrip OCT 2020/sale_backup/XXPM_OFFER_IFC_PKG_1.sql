...
BEGIN
  ...
  BEGIN
    ...
    WHEN OTHERS THEN
        P_ERR_CODE := SQLCODE;
        P_ERR_MSG  := SUBSTR (SQLERRM, 1, 2000);
        UPDATE XXDM_SALES_MILESTONES set STATUS_FLAG='F',ERR_DESC=P_ERR_CODE || '-' || P_ERR_MSG
        WHERE booking_number = i.booking_number
        AND rowid  = i.rowid;
    END;
    END LOOP; 
