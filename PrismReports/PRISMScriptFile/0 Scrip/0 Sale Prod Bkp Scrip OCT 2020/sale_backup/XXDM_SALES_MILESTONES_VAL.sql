...
set define off;
  ...
  CREATE OR REPLACE EDITIONABLE PROCEDURE "XXDM_SALES_MILESTONES_VAL" AS

    CURSOR cur_sales_miles IS
    SELECT
        *
    FROM
        xxdm_sales_milestones
    WHERE
        nvl(status_flag, 'N') = 'N';
...
--    BEGIN
        ...
        IF l_offer_number > 0 THEN
           ...
           /* UPDATE xxdm_sales_milestones
            SET
                p_installment_type = l_installment_type,
