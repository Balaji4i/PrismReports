--------------------------------------------------------
--  DDL for View XXPM_BOOKING_MILESTONES_V
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE VIEW "XXPM_BOOKING_MILESTONES_V" ("ROW_ID", "BOOKING_HDR_ID", "BOOKING_MS_DTL_ID", "MILESTONE_TYPE", "MS_DTL_ID", "PL_MOD_ID", "SEQ_NUMBER", "INSTALLMENT_TYPE", "INSTALLMENT_TYPE_DESC", "INSTALLMENT_PCT", "INSTALLMENT_AMOUNT", "UOM", "QUANTITY", "RATE", "WAVE_OFF", "INCLUDED_IN_RATE", "INSTALLMENT_METHOD", "INSTALLMENT_METHOD_DESC", "INSTALLMENT_EVENT", "PERIOD_OCCURENCE", "PERIOD_VALUES", "PERIOD_DURATION", "CHARGE_TYPE", "CHARGE_TYPE_DESC", "PAYMENT_TERM", "REMARKS", "DUE_DAYS", "DUE_DAYS_FROM", "DUE_DATE", "ATTRIBUTE_CATEGORY", "ATTRIBUTE1", "ATTRIBUTE2", "ATTRIBUTE3", "ATTRIBUTE4", "ATTRIBUTE5", "ATTRIBUTE6", "ATTRIBUTE7", "ATTRIBUTE8", "ATTRIBUTE9", "ATTRIBUTE10", "CREATED_BY", "CREATION_DATE", "LAST_UPDATED_BY", "LAST_UPDATE_DATE", "LAST_UPDATE_LOGIN", "SOURCE_FUNC_ID", "SOURCE_FUNC_REF_ID", "BILLING_ID", "STATUS", "INVOICE_ID", "INVOICE_NUMBER", "TAX_RATE", "TAX_CODE", "TAX_AMOUNT", "BASEAMOUNT", "CARPARKING", "SOURCE_NUMBER", "CUST_TYPE", "MILESTONES_STATUS", "INSTALLMENT_PCT_R", "INSTALLMENT_AMOUNT_R") AS 
  SELECT DISTINCT xbm.rowid,
    xbm.booking_hdr_id,
    ...
    xbm.INSTALLMENT_EVENT  AS installment_event, --(SELECT installment_event FROM xxpm_milestone_dtl WHERE ms_dtl_id = xbm.ms_dtl_id),
    (SELECT period_occurence
    FROM xxpm_milestone_dtl
    WHERE ms_dtl_id = xbm.ms_dtl_id
    ) AS period_occurence,
    ...
    xbm.INSTALLMENT_PCT_R,
    xbm.INSTALLMENT_AMOUNT_R
  ...
  WHERE lk2.lookup_type_name (+) = 'MILESTONE_METHOD'
