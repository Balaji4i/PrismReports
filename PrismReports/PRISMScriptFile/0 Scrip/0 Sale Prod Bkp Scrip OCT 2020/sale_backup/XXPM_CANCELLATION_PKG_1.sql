--------------------------------------------------------
--  DDL for Package Body XXPM_CANCELLATION_PKG
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "XXPM_CANCELLATION_PKG" 
IS
...
BEGIN
        ...
        IF ( l_forfiet_amount != 0) THEN
            ...
            COMMIT;
            begin
            XXPM_CANCELLATION_PKG.XXPM_CREATE_AUTO_BILLING(P_PRIMARY_ID,P_FUNC_ID,'DEB_MO',p_login_user,l_P_ERR_MSG);
...
if(l_cmc_amount<>0) then        
        ...
        dbms_output.put_line('next installemnt Id lms_dtlId==>' || lms_dtlid);
            ...
            COMMIT;
            begin
            XXPM_CANCELLATION_PKG.XXPM_CREATE_AUTO_BILLING(P_PRIMARY_ID,P_FUNC_ID,'CMC',p_login_user,l_P_ERR_MSG);
...
if(p_type='DEB_MO') then
        ...
        and bms.BILLING_ID is null
        and bms.SOURCE_FUNC_REF_ID=p_primary_id;
elsif(p_type='CMC')then
        Select 
        bh.org_id, 
        pro.PROJ_ID,
        BOOKING_MS_DTL_ID
