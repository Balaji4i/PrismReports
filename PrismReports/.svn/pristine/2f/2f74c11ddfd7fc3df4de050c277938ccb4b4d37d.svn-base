...
SELECT xb.billing_date invoice_date
      ...
      IS
         SELECT  booking_dtl_id
               ...
               ,xpd.property_id
               ,xpd.building_id
               ,nvl(xpd.booking_amount,0) as booking_amount
               ,nvl(xpd.DISC_AMOUNT,0) as DISC_AMOUNT
               ,nvl(xpd.booking_amount,0)-nvl(xpd.DISC_AMOUNT,0)+nvl(tax_amount,0) as TOTAL_SPA
               ,xpd.disc_pct
...
------*******
                ...
                l_line_amount:=GET_LINE_AMT(cr1.AMOUNT,cr2.TAX_CODE, NVL(cr2.TAX_PERC,0));
                l_tax_amount:=GET_TAX_AMT(cr1.AMOUNT,cr2.TAX_CODE, NVL(cr2.TAX_PERC,0));
--
--                        IF cr1.milestone_type = 'MS'
--                        THEN
--                           l_tot_amt   :=
...
--                           l_line_amount :=NVL(cr1.amount,0)- l_tax_amount;
--                        ELSE
--                           l_tax_amount := NVL(cr1.tax_amount,0);
--                           l_line_amount :=NVL(cr1.amount,0)- l_tax_amount;
--                        END IF;
