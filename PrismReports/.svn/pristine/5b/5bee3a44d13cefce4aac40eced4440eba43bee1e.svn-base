--------------------------------------------------------
--  DDL for Procedure XXDM_CREATE_BOOKING
--------------------------------------------------------
set define off;

  CREATE OR REPLACE EDITIONABLE PROCEDURE "XXDM_CREATE_BOOKING" 
AS
  ...
  I_OFFER_HDR_ID       NUMBER;
  I_BOOK_TYPE          VARCHAR2(60);
  I_BOOKING_HDR_ID     NUMBER;
  I_BOOKING_EXE_BY     NUMBER;
  I_booking_number     VARCHAR2(60);
  v_err_flag           VARCHAR2(60);
  ...
  CURSOR offer_header 
  IS 
  select * from xxpm_offer_header  where Migrated_data ='I';
...
FOR offer IN offer_header
loop
 dbms_output.put_line( ' loop 1  ');

 I_BOOKING_HDR_ID :=XXPM_BOOKING_HEADER_S.nextval;
  V_BOOKING_HDR_ID:=I_BOOKING_HDR_ID;


I_OFFER_HDR_ID:=offer.OFFER_HDR_ID;
I_booking_number :=offer.offer_number;
  ...
  BEGIN
    ...
    WHEN no_data_found THEN
      v_errm := SQLCODE || '-' || SQLERRM;
          v_err_flag := 'E';
                        DBMS_OUTPUT.PUT_LINE('exception no data found 21');


    WHEN OTHERS THEN
      v_errm := SQLCODE || '-' || SQLERRM;
          v_err_flag := 'E';
                        DBMS_OUTPUT.PUT_LINE('exception no data found 20');
...
--          p_errm:=v_errm;
  ...
  WHEN no_data_found THEN
    v_errm := SQLCODE || '-' || SQLERRM;
          v_err_flag := 'E';
                                  DBMS_OUTPUT.PUT_LINE('exception no data found 16');


  WHEN OTHERS THEN
    v_errm := SQLCODE || '-' || SQLERRM;
          v_err_flag := 'E';
    DBMS_OUTPUT.PUT_LINE('exception no data found 17 '||v_errm);
  ...
  WHEN OTHERS THEN
    v_errm := SQLCODE || '-' || SQLERRM;
          v_err_flag := 'E';
                        DBMS_OUTPUT.PUT_LINE('exception no data found 15');
  ...
  WHEN no_data_found THEN
    v_errm := SQLCODE || '-' || SQLERRM;
          v_err_flag := 'E';
                        DBMS_OUTPUT.PUT_LINE('exception no data found 12');

                        v_CUST_CONTACT_ID := null;


  WHEN OTHERS THEN
    v_errm := SQLCODE || '-' || SQLERRM;
          v_err_flag := 'E';
                        DBMS_OUTPUT.PUT_LINE('exception no data found 13');
  ...
  WHEN no_data_found THEN
    v_errm := SQLCODE || '-' || SQLERRM;
          v_err_flag := 'E';
              ...
              DBMS_OUTPUT.PUT_LINE('exception no data found  7');


  WHEN OTHERS THEN
    v_errm := SQLCODE || '-' || SQLERRM;
          v_err_flag := 'E';
              DBMS_OUTPUT.PUT_LINE('exception no data found 8');
  ...
  WHEN no_data_found THEN
    v_errm := SQLCODE || '-' || SQLERRM;
          v_err_flag := 'E';
    DBMS_OUTPUT.PUT_LINE('exception no data found   5');

  WHEN OTHERS THEN
    v_errm := SQLCODE || '-' || SQLERRM;
          v_err_flag := 'E';
    DBMS_OUTPUT.PUT_LINE('exception no data found   6');
  ...
  WHEN no_data_found THEN
    v_errm := SQLCODE || '-' || SQLERRM;
          v_err_flag := 'E';
    DBMS_OUTPUT.PUT_LINE('exception no data found  3 ');

  WHEN OTHERS THEN
    v_errm := SQLCODE || '-' || SQLERRM;
          v_err_flag := 'E';
              DBMS_OUTPUT.PUT_LINE('exception no data found   4');
  ...
  WHEN no_data_found THEN
    v_errm := SQLCODE || '-' || SQLERRM;
          v_err_flag := 'E';
    DBMS_OUTPUT.PUT_LINE('exception no data found  1 ');

  WHEN OTHERS THEN
    v_errm := SQLCODE || '-' || SQLERRM;
          v_err_flag := 'E';
              DBMS_OUTPUT.PUT_LINE('exception no data found  2 ');
...
--  WHERE OFFER_HDR_ID=I_OFFER_HDR_ID;
  ...
  UPDATE xxpm_offer_header  SET  Migrated_data ='Y' WHERE OFFER_HDR_ID =I_OFFER_HDR_ID;
  commit;
  end loop;
