...
--------------------------------------------------------
  ...
  SELECT bh.ROWID
        ...
        , (SELECT property_name
            ...
            WHERE property_id = bh.property_id)
            property_name
        ,bh.offer_hdr_id
        ,xoh.offer_number
        ,bh.block_id
        ,xbl.block_number
        , (SELECT xsc.cust_id
             FROM xxstg_customer xsc
            WHERE xoh.customer_id = xsc.cust_id)
            cust_id
        , (SELECT xsc.customer_name
             FROM xxstg_customer xsc
            WHERE xoh.customer_id = xsc.cust_id)
            customer_name
        ,bh.booking_number
		...
		xoh.NATIONALITY,xoh.EMIRATES_ID
   ...
   WHERE     xoh.offer_hdr_id(+) = bh.offer_hdr_id
...
ORDER BY bh.booking_hdr_id DESC
