...
IS
         ...
         THEN
            INSERT     (bank_acnt_id
                ...
                VALUES (b.bank_acnt_id
                       ...
                       ,b.bank_account_name);
      END LOOP;
      UPDATE_NEXT_SCHEDULE_TIME('BankAccountDetails');
      COMMIT;
   END bank_accounts;
   ...
   BEGIN
      ...
      LOOP
         ...
         END IF;
      END LOOP;
      UPDATE_NEXT_SCHEDULE_TIME('Customer');
      COMMIT;
   END customers;
...
--                     AND b.cust_site_id IS NOT NULL
      ...
      END LOOP;
      UPDATE_NEXT_SCHEDULE_TIME('CustomerSites');
      COMMIT;
   END customer_site;
         ...
         THEN
            INSERT     (loc_id
                ...
                VALUES (b.loc_id
                       ...
                       ,b.po_box);
      END LOOP;
      UPDATE_NEXT_SCHEDULE_TIME('Location');
      COMMIT;
   END locations;
         ...
         THEN
            INSERT     (org_id
                ...
                VALUES (b.org_id
                       ...
                       ,b.last_update_login);
      END LOOP;
      UPDATE_NEXT_SCHEDULE_TIME('OrganizationUnits');
      COMMIT;
   END organizations;
         ...
         THEN
            INSERT     (term_id
                ...
                VALUES (b.term_id
                       ...
                       ,b.last_update_login);
      END LOOP;
      UPDATE_NEXT_SCHEDULE_TIME('PaymentTerm');
      COMMIT;
   END payment_terms;
         ...
         THEN
            INSERT     (cust_contact_id
                ...
                VALUES (b.cust_contact_id
                       ...
                       ,b.last_update_login);
      END LOOP;
      UPDATE_NEXT_SCHEDULE_TIME('CustomerContact');
      COMMIT;
         ...
         THEN
            INSERT     (emp_id
                ...
                VALUES (b.emp_id
                       ...
                       ,b.position_id);
      END LOOP;
      UPDATE_NEXT_SCHEDULE_TIME('Employee');
      COMMIT;
   END employee;
         ...
         THEN
            INSERT     (vend_id
                ...
                VALUES (b.vend_id
                       ...
                       ,b.last_update_login);
      END LOOP;
      UPDATE_NEXT_SCHEDULE_TIME('Vendor');
      COMMIT;
   END vendor;
         ...
         THEN
            INSERT     (vend_contact_id
                ...
                VALUES (b.vend_contact_id
                       ...
                       ,b.fax_number);
      END LOOP;
      UPDATE_NEXT_SCHEDULE_TIME('VendorContact');
      COMMIT;
   END vendor_contact;
         ...
         THEN
            INSERT     (vend_site_id
                ...
                VALUES (b.vend_site_id
                       ...
                       ,b.last_update_login);
      END LOOP;
      UPDATE_NEXT_SCHEDULE_TIME('VendorSite');
      COMMIT;
   END vendor_site;
         ...
         THEN
            INSERT     (proj_id
                ...
                VALUES (b.proj_id
                       ...
                       ,b.active_yn);
      END LOOP;
      UPDATE_NEXT_SCHEDULE_TIME('Projects');
      COMMIT;
   END projects;
...
--   END receipt_status;
   ...
   BEGIN
      ...
      END LOOP;
      UPDATE_NEXT_SCHEDULE_TIME('ReceiptMethod');
      COMMIT;
   END receipt_method;

   PROCEDURE organizations_new (p_xml_data CLOB)
   AS
      acct_doc XMLTYPE := xmltype.createxml (p_xml_data);
      ...
      IS
                 SELECT org_id
                   ...
                   FROM XMLTABLE (
                           ...
                           COLUMNS org_id        VARCHAR2 (240) PATH 'ORGANIZATION_ID'
                                  ,org_name      VARCHAR2 (240) PATH 'NAME'
                                  ,org_name_tl   VARCHAR2 (240) PATH 'PROJECT_NAME'
--                                  ,short_code    VARCHAR2 (150) PATH 'SHORT_CODE'
                                  ,legal_entity_id VARCHAR2 (240) PATH 'LEGAL_ENTITY_ID'
--                                  ,ledger_id     VARCHAR2 (240) PATH 'LEDGER_ID'
                                  ,ledger_name   VARCHAR2 (50) PATH 'LEGAL_NAME'
--                                  ,ledger_gl_code VARCHAR2 (20) PATH 'LEDGER_GL_CODE'
--                                  ,org_gl_code   VARCHAR2 (20) PATH 'ORG_GL_CODE'
                                  ,orig_system_ref VARCHAR2 (240) PATH 'ORGANIZATION_ID'
                                  ,active_yn     VARCHAR2 (1) PATH 'STATUS');
   BEGIN
      OPEN c_org;
      ...
      LOOP
         MERGE INTO xxstg_organizations a
              USING (SELECT l_org_tbl (i).org_id org_id
                           ,l_org_tbl (i).org_name org_name
                           ,l_org_tbl (i).org_name_tl org_name_tl
                           ,l_org_tbl (i).short_code short_code
                           ,l_org_tbl (i).orig_system_ref orig_system_ref
                           ,l_org_tbl (i).active_yn active_yn
                           ,l_org_tbl (i).created_by created_by
...
SET
    ...
    next_schedule_time = SYSDATE + ( ( frequency_min - 245 ) / 24 / 60 )
...
PROCEDURE UPDATE_RUN_STATUS (SCHEDULER_P varchar2, ACTION_P varchar2)
AS
BEGIN 
update XXFND_SCHEDULE_T set RUN_STATUS = ACTION_P where SCHEDULE_NAME = SCHEDULER_P; 
null;
