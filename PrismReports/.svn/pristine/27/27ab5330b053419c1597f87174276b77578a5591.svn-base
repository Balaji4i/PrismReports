...
--        dbms_output.put_line('action_flag'|| i.action_flag); 
            update XXPM_PROPERTY_UNITS set STATUS = 'ALOT' where UNIT_NUMBER = i.carparking;
            commit;
...
procedure update_cp_unit( cp_number in VARCHAR2 , p_booking_hdr_id in number)
as
cp_unit_id number;
p_unit_id number;
...
begin
SELECT UNIT_ID, PROPERTY_ID into p_unit_id, p_property_id FROM xxpm_booking_detail where booking_hdr_id=p_booking_hdr_id;

SELECT UNIT_ID into cp_unit_id FROM XXPM_PROPERTY_UNITS where rownum=1 and PROPERTY_ID=p_property_id and UNIT_NUMBER = cp_number;

update XXPM_PROPERTY_UNITS set STATUS = 'ALOT' where UNIT_NUMBER = cp_number;

Insert into XXPM_PROPERTY_CARPARKS
(CARPARK_ID,UNIT_ID,CP_UNIT_ID,ALLOT_TYPE,CREATED_BY,CREATION_DATE,LAST_UPDATED_BY,LAST_UPDATE_DATE,LAST_UPDATE_LOGIN) 
values 
(XXPM_AREA_ID_S.nextval,p_unit_id,cp_unit_id,'PAID','anonymous',to_timestamp(sysdate),'anonymous',to_timestamp(sysdate),'anonymous');
